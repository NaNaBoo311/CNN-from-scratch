...
xMap<K,V>::xMap(const xMap<K,V>& map){
    //YOUR CODE IS HERE
    //CopyMapFrom without removeInternalData
    this->hashCode = map.hashCode;
    this->loadFactor = map.loadFactor;
    this->valueEqual = map.valueEqual;
    this->keyEqual = map.keyEqual;

    //SHOULD NOT COPY: deleteKeys, deleteValues => delete ONLY 1 TIME in map if needed
    this->deleteKeys = 0;
    this->deleteValues = 0;
...
V xMap<K,V>::put(K key, V value){
   ...
   DLinkedList<Entry *> &list = this->table[index];
    ...
    int new_index = this->hashCode(key, capacity);
    this->table[new_index].add(new Entry(key, value));
        ...
        for(int idx=0; idx < this->capacity; idx++){
            ...
            for(auto &Entry: list) {
                if (keyEQ(Entry->key, key)) {
                    ...
                    list.removeItem(Entry, deleteEntry);
                    count--;
                    return value;
                }
            }
...
void xMap<K,V>::removeInternalData(){
    ...
    for(int idx=0; idx < this->capacity; idx++){
        DLinkedList<Entry*>& list = this->table[idx];
        for(auto pEntry: list) {
          delete pEntry; //delete T data
        }
        list.clear(); //delete Node
    }
