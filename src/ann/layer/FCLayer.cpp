...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
    ...
    if (DY.dimension() == 1)
    {
        this->m_unSample_Counter += 1;
        if (this->m_trainable == true)
        {
            this->m_aGrad_W = xt::linalg::outer(DY, this->m_aCached_X);
        }
    }
    ...
    else if (DY.dimension() > 1)
    {
        ...
        // Calculate DW if trainable
        if (this->m_trainable == true)
        {
            int N = DY.shape()[0];

            xt::xarray<double> temp_m_aGrad_W = xt::zeros<double>({N, int(DY.shape()[1]), int(this->m_aCached_X.shape()[1])});
            for (int i = 0; i < N; i++)
            {
                xt::xarray<double> DY_i = xt::view(DY, i);
                xt::xarray<double> X_i = xt::view(this->m_aCached_X, i);
                xt::view(temp_m_aGrad_W, i) = xt::linalg::outer(DY_i, xt::transpose(X_i));
            }

            this->m_aGrad_W = xt::mean(temp_m_aGrad_W, {0});
        }
    }
